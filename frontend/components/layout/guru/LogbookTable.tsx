...
import { User, Calendar, BookOpen, Eye, MoreHorizontal, Trash2, CheckSquare } from "lucide-react";
...
import { LogbookVerificationModal } from '../guru/update/LogbookModal';
...
export function LogbookTable() {
    ...
    // State untuk modal verifikasi (jika masih diperlukan)
    const [selectedLogbook, setSelectedLogbook] = useState<LogbookEntry | null>(null);
    const [showVerificationModal, setShowVerificationModal] = useState(false);
    
    // State untuk modal update dengan status
    ...
    // Handler untuk membuka modal edit (tanpa status)
    const handleEdit = (entry: LogbookEntry) => {
        setSelectedUpdateLogbook(entry);
        setShowStatusUpdate(false); // Tidak menampilkan update status
    ...
    const handleUpdateStatus = (entry: LogbookEntry) => {
        setSelectedUpdateLogbook(entry);
        setShowStatusUpdate(true); // Menampilkan update status
        setShowUpdateModal(true);
    };

    // Handler untuk update logbook melalui modal
    ...
    // Handler untuk verifikasi (menggunakan modal terpisah jika masih diperlukan)
    const handleVerification = async (data: any) => {
        if (!selectedLogbook) return false;

        try {
            const token = localStorage.getItem('access_token');
            const response = await fetch(
                `http://localhost:8000/api/logbook/${selectedLogbook.id}/verifikasi`,
                {
                    method: 'PUT',
                    headers: {
                        'Authorization': `Bearer ${token}`,
            ...
            if (result.success) {
                ...
                alert(result.message || 'Gagal menyimpan verifikasi');
                return false;
            }
        } catch (err) {
    ...
    return (
        <>
            ...
            {/* Modal Verifikasi (Optional - jika masih diperlukan) */}
            {showVerificationModal && (
                <LogbookVerificationModal
                    ...
                    title="Verifikasi Logbook Siswa"
                />
            )}

            {/* Modal Update - Multi-purpose (Edit Logbook atau Update Status) */}
            <LogbookUpdateModal
