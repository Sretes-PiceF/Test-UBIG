...
const handleEdit = (logbook: LogbookData) => {
    ...
    const handleDelete = async (logbook: LogbookData) => {
        // Cek apakah magang masih berlangsung
        if (!canAddLogbook) {
            alert('Anda tidak dapat menghapus logbook karena magang sudah selesai atau belum dimulai');
            return;
        }
        ...
        if (confirm(`Apakah Anda yakin ingin menghapus logbook tanggal ${logbook.tanggal_formatted}?`)) {
            try {
    ...
    const handleAddLogbook = () => {
        if (!canAddLogbook) {
            alert('Anda tidak dapat menambah logbook karena magang belum dimulai atau sudah selesai');
            return;
        }
    ...
    const getStatusMessage = () => {
        if (statusMagang === 'pending' || statusMagang === 'diterima') {
            return {
                type: 'info',
                message: 'Magang Anda belum dimulai. Anda dapat menambah logbook setelah magang berlangsung.'
            };
        } else if (statusMagang === 'selesai' || statusMagang === 'ditolak' || statusMagang === 'dibatalkan') {
            return {
                type: 'warning',
                message: 'Magang Anda sudah selesai/dibatalkan. Anda tidak dapat menambah atau mengedit logbook.'
            };
        }
        return null;
    };
                                ...
                                <div key={log.id} className="grid grid-cols-12 gap-4 px-6 py-5 hover:bg-gray-50 transition-colors">
                                    ...
                                    <div className="col-span-4 space-y-3">
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-900 line-clamp-2 leading-relaxed">{log.kegiatan}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs font-semibold text-gray-500 mb-1">Kendala:</p>
                                            <p className="text-sm text-gray-900 line-clamp-2 leading-relaxed">{log.kendala}</p>
                                        </div>
                                    </div>

                                    {/* Status */}
                                    <div className="col-span-2 flex items-center justify-center">
                                        <span className={`inline-flex items-center px-3 py-1.5 text-xs font-semibold rounded-full ${getStatusClass(log.status_verifikasi)}`}>
                                    ...
                                    <div className="col-span-3 space-y-2">
                                        {log.catatan_guru ? (
                                            <div className="bg-blue-50 border border-blue-100 p-2.5 rounded-lg">
                                                ...
                                                <p className="text-xs text-gray-700 leading-relaxed line-clamp-2">{log.catatan_guru}</p>
                                            </div>
                                        ) : null}
                                        {log.catatan_dudi ? (
                                            <div className="bg-purple-50 border border-purple-100 p-2.5 rounded-lg">
                                                ...
                                                <p className="text-xs text-gray-700 leading-relaxed line-clamp-2">{log.catatan_dudi}</p>
                                            </div>
                                        ) : null}
                ...
                {!loading && logbooks.length > 0 && (
                    <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between bg-gray-50">
                        <p className="text-sm text-gray-600">
                            Menampilkan <span className="font-semibold">{pagination.from}</span> sampai <span className="font-semibold">{pagination.to}</span> dari <span className="font-semibold">{pagination.total}</span> logbook
                        </p>
